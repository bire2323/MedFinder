...
const SidebarNavigation = ({ activeSection, onSectionClick, theme }) => {
  ...
  return (
    <nav className="flex flex-wrap lg:flex-col gap-2 overflow-x-auto pb-4 lg:pb-0 lg:w-64 shrink-0 h-fit sticky top-4 snap-x no-scrollbar">
      {sections.map((section) => (
        <button
          key={section.id}
          onClick={() => onSectionClick(section.id)}
          className={`flex-none lg:w-full flex items-center justify-center lg:justify-start gap-3 px-6 lg:px-4 py-4 rounded-3xl font-bold transition-all duration-300 snap-center ${activeSection === section.id
