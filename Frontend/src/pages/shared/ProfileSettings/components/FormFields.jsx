...
export const InputField = ({ label, icon, value, onChange, placeholder, error, readOnly, tooltip, theme }) => (
  <div className="space-y-2 relative group">
    <label className="text-[10px] uppercase font-black text-slate-400 flex items-center gap-2 ml-1">
      {icon} {label}
      {readOnly && (
        <span className={`px-2 py-0.5 flex items-center gap-1 ${theme?.badgeBg || 'bg-blue-100'} ${theme?.textPrimary || 'text-blue-600'} text-[9px] rounded-full border ${theme?.borderPrimary || 'border-blue-200'} uppercase absolute right-0 font-bold`}>
          Read Only
        </span>
      )}
    </label>
    <div className="relative">
      <input
        ...
        readOnly={readOnly}
        className={`w-full p-4 border border-transparent rounded-2xl text-sm font-medium outline-none transition-all ${error
...
export const SelectField = ({ label, value, options, onChange, error, theme }) => (
  <div className="space-y-2">
    ...
    <select
      ...
      className={`w-full p-4 bg-slate-50 dark:bg-gray-800 border-transparent hover:border-slate-300 border rounded-2xl text-sm font-medium outline-none transition-all ${error ? "focus:ring-2 focus:ring-red-500 bg-red-50 dark:bg-red-900/10" : `focus:ring-2 ${theme?.ringFocus || 'focus:ring-blue-500'}`
        }`}
    >
