...
const ProfileSettings = ({ initialData, type = "pharmacy", onUpdateSuccess }) => {
  ...
  const normalizeData = (initialData) => {
    ...
    if (data.working_hour !== undefined) {
      data.working_hour = parseWorkingHour(data.working_hour);
    } else {
      data.working_hour = {};
    }
  ...
  const [previewOfficialLicense, setPreviewOfficialLicense] = useState(initialData?.official_license_upload_url || null);
  ...
  const handleSubmit = async () => {
    setIsDirty(false);
    console.log("Submitting form with data:", formData, "and files:", files);
    const ok = await updateProfile(formData, files);
    if (ok && onUpdateSuccess) {
      document.getElementById("result")?.scrollIntoView({ behavior: "smooth", block: "center" });
      setFormData(normalizeData(initialData));
      console.log("AFTER UPDATE initialData:", initialData);
      onUpdateSuccess();
    }
  };
  ...
  return (
    <div className="">
      ...
      {activeSection === "" ? (
        <>
          <nav className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 w-full space-y-2">
            {sections.map((section) => (
              <div className="h-40px  bg-white dark:bg-gray-800 px-1 py-2 md:px-2 md:py-3 mx-3 md:mx-5" key={section.id}>
                ...
                <button
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-bold transition-all ${activeSection === section.id
        ...
        <div className="flex-1 space-y-12 max-w-4xl">
          ...
          {activeSection === "general" && (
            <SectionWrapper id="general" title={t("Settings.GeneralInfo")} theme={theme} setactiveSection={setActiveSection}>
          ...
          {activeSection === "license" && (
            <SectionWrapper id="license" title={t("Settings.LicenseVerification")} theme={theme} setactiveSection={setActiveSection}>
              <div className="bg-slate-50 dark:bg-gray-800/50 p-6 rounded-xl border border-dashed border-gray-400">
                ...
                <div className="mt-8">
                  <FileInput
                    ...
                    currentFile={formData.official_license_upload_url || previewOfficialLicense}
                  />
                </div>
              </div>
            </SectionWrapper>
          ...
          {activeSection === "contact" && (
            ...
            <SectionWrapper id="contact" title={t("Settings.ContactInfo")} theme={theme} setactiveSection={setActiveSection}>
              <div className="flex flex-wrap">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <InputField
                    label={t("Settings.contact_phone")}
                    icon={<Phone size={14} />}
              ...
              <button onClick={() => setactiveSection("")}>
                <FaBackward className="cursor-pointer text-slate-700 hover:text-slate-900 dark:text-slate-100 dark:hover:text-slate-50" />
              </button>
            </SectionWrapper>
          )}
          ...
          {activeSection === "location" && (
            ...
            <SectionWrapper id="location" title={t("Settings.LocationAddress")} theme={theme} setactiveSection={setActiveSection}>
          ...
          {activeSection === "availability" && (
            ...
            <SectionWrapper id="availability" title={t("Settings.Availability")} theme={theme} setactiveSection={setActiveSection}>
          ...
          {activeSection === "media" && (
            ...
            <SectionWrapper id="media" title={t("Settings.MediaIdentity")} theme={theme} setactiveSection={setActiveSection}>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <AlertTriangle className="text-red-500" size={24} />
                      <h3 className="text-[11px] md:text-xl font-black uppercase text-red-600 tracking-tight">{t("Settings.DangerZone")}</h3>
                    </div>
                    <p className="text-[10px] md:text-sm text-red-700/70 dark:text-red-400 max-w-lg">
                      {t("Settings.DeactivationWarning")}
                    </p>
                  </div>
                  <button className="bg-red-600 text-white px-2 md:px-8 py-1 md:py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] md:text-xs hover:bg-red-700 shadow-xl shadow-red-500/20 transition-all flex items-center gap-2">
        ...
        {isDirty && (
          ...
          >
            <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl p-4 rounded-3xl border border-blue-200 dark:border-gray-700 shadow-2xl flex items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => { setFormData(normalizeData(initialData)); setFiles({}); setPreviewLogo(initialData?.logo_url); }}
                  className="px-2 md:px-6 py-1 md:py-3 rounded-2xl text-[10px] md:text-xs font-semibold md:font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-gray-800 transition-colors"
                >
                  {t("Common.Discard")}
                </button>
                <button
                  ...
                  disabled={loading}
                  className="bg-blue-600 text-white px-2 py-1 md:px-8 md:py-3 rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:bg-blue-700 shadow-lg shadow-blue-500/20 disabled:opacity-50 transition-all"
                >
...
const SectionWrapper = ({ id, title, children, theme, setactiveSection }) => (
  <section id={id} className="bg-white dark:bg-gray-900 p-8 rounded-xl border border-gray-200 dark:border-gray-500 shadow-sm scroll-mt-24">
    <div className="flex items-center justify-between gap-4 mb-3 md:mb-8">
      <button onClick={() => setactiveSection("")} className="hover:bg-slate-200 dak:hover:bg-slate-700 p-1.5 md:p-2 ">
        <ChevronLeft size={16} className="cursor-pointer text-slate-700 hover:text-slate-900 dark:text-slate-100 dark:hover:text-slate-50" />
      </button>
      <div className="flex items-center gap-3">
        <div className={`w-2 h-8 ${theme.bgPrimary} rounded-full`} />
        <h3 className="text-sm md:text-2xl font-black uppercase tracking-tight">{title}</h3>
      </div>
    </div>
    {children}
  </section>
...
const InputField = ({ label, icon, value, onChange, placeholder, error }) => (
  <div className="space-y-2">
    ...
    <input
      ...
      className={`w-full p-4 bg-slate-50 dark:bg-gray-800 border-none rounded-2xl text-sm font-medium outline-none focus:ring-2 transition-all ${error ? "focus:ring-red-500 bg-red-50 dark:bg-red-900/10" : "focus:ring-blue-500"
        }`}
    />
