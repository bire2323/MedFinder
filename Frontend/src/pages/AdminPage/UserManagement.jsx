...
function getRoleBadgeClass(role) {
  switch (role) {
    case 'patient':
      return 'bg-gray-100 text-gray-700 border border-gray-200 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300';
    case 'hospital_agent':
      return 'bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-900/40 dark:border-indigo-800 dark:text-indigo-400';
    case 'pharmacy_agent':
      return 'bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/40 dark:border-emerald-800 dark:text-emerald-400';
    default:
      return 'bg-gray-100 text-gray-700 border border-gray-200 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300';
  }
}
...
function getStatusBadgeClass(status) {
  if (status === true || status === 'active') return 'bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/40 dark:border-emerald-800 dark:text-emerald-400';
  return 'bg-gray-100 text-gray-600 border border-gray-200 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400';
}
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:ring-2 focus:ring-green-500 focus:border-green-400 outline-none transition"
            />
          </div>
          <select
            value={roleFilter}
            onChange={(e) => setRoleFilter(e.target.value)}
            className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-green-500 outline-none transition"
          >
      ...
      {loading ? (
        ...
        <>
          {/* Desktop Table */}
          <div className="hidden md:block bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md">
            <div className="px-6 pt-5 pb-3">
              <h3 className="font-semibold text-gray-900 dark:text-white">{t("Admin.Users")}</h3>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-900 border-y border-gray-200 dark:border-gray-700">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-300">{t("Admin.User")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-300">{t("Admin.Role")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-300">{t("Admin.Status")}</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-300">{t("Admin.Actions")}</th>
            ...
            {filteredUsers.length === 0 && (
              <div className="px-6 py-14 text-center text-gray-600 dark:text-gray-400 text-sm">
          ...
          <div className="md:hidden space-y-3">
            {filteredUsers.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-md"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="relative">
                      ...
                      {openDropdownId === user.id && (
                        <div className="absolute right-0 top-8 z-20 w-48 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg py-1">
                          <button
                            type="button"
                            className="w-full flex items-center gap-2 px-3 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                            onClick={() => openEdit(user)}
                  ...
                  <div className="border-t border-gray-100 dark:border-gray-700 pt-3 mt-3">
                    <button
                      onClick={() => setExpandedUserId(isExpanded ? null : user.id)}
                      className="w-full flex items-center justify-center gap-1 text-xs font-semibold text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors"
                    >
                      {isExpanded ? (
                        <><ChevronDown className="size-4" /> Hide Details</>
                      ) : (
                    ...
                    {isExpanded && (
                      <div className="mt-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg p-3 text-sm space-y-3">
                        ...
                        <div>
                          <p className="text-gray-500 dark:text-gray-400 font-semibold mb-1 text-xs uppercase tracking-wider">System Info</p>
                          <p className="text-gray-800 dark:text-gray-200"><span className="text-gray-500">Created:</span> {new Date(user.created_at).toLocaleString()}</p>
                          <p className="text-gray-800 dark:text-gray-200"><span className="text-gray-500">Last Seen:</span> {user.last_seen_at ? new Date(user.last_seen_at).toLocaleString() : 'N/A'}</p>
                        </div>
                      </div>
            ...
            {filteredUsers.length === 0 && (
              <div className="py-14 text-center text-gray-600 dark:text-gray-400 text-sm">
                {t("Admin.NoUsersFound")}
              </div>
            )}
          </div>
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex justify-center items-center gap-2">
          <button
            ...
            disabled={page === 1}
            className="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm font-semibold text-gray-700 dark:text-gray-300 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          >
            {t("common.previous")}
          </button>
          ...
          <div className="flex gap-1 overflow-x-auto px-1">
            {getVisiblePages().map((p, i) =>
              p === '...' ? (
                <span key={i} className="px-2 py-2 text-gray-500 text-sm">...</span>
              ) : (
                <button
                  key={p}
                  onClick={() => setPage(p)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full p-7 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-1.5 bg-green-50 dark:bg-gray-800 rounded-lg border border-green-200 dark:border-gray-700">
                <Edit className="size-4 text-green-700 dark:text-green-500" />
              </div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t("Admin.EditUserRole")}</h3>
            </div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm mb-6 focus:ring-2 focus:ring-green-500 outline-none transition"
            >
              <option value=""></option>
              <option value="patient">{t("Admin.Patient")}</option>
              <option value="hospitalAgent">{t("Admin.HospitalAgent")}</option>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                type="button"
                className="px-5 py-2.5 rounded-xl bg-green-600 text-white text-sm font-bold hover:bg-green-700 transition-colors shadow-md active:scale-95 border border-green-600"
