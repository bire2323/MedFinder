...
export const useRegistrationStore = create((set, get) => ({
  ...
  validateStep2: () => {
    ...
    // Working hour validation for hospital
    if (registrationType === 'hospital') {
      if (!formData.workingHour || typeof formData.workingHour !== 'object' || Object.keys(formData.workingHour).length === 0) {
        errors.workingHour = isAmharc ? "የስራ ሰዓት ያስገቡ" : 'Working hours are required';
      }
    }
  ...
  validateStep3Pharmacy: () => {
    ...
    // Working hours validation - check if at least one hour is selected across all days
    if (!formData.workingHour || typeof formData.workingHour !== 'object') {
      errors.workingHour = isAmharc ? "የስራ ሰዓት ያስገቡ" : 'Working hours are required';
    } else {
      const hasSelectedHours = Object.values(formData.workingHour).some(hours => hours && hours.length > 0);
      if (!hasSelectedHours) {
        errors.workingHour = isAmharc ? "ቢያንስ አንድ ሰዓት ያስገቡ" : 'Please select at least one working hour';
      }
    }
